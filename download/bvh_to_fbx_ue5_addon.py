...
# ============================================================================
# RETARGETING ENGINE v6.0 — DIRECT POSE COPY WITH AXIS CORRECTION
# ============================================================================
# v5.0 BUG: The armature-local delta approach (ue5_rest.inv() @ delta_parent.inv()
# @ delta_bone @ ue5_rest) is mathematically correct, BUT in practice it fails
# because:
#
# 1. BVH import with wrong axis_forward/axis_up produces INCORRECT bone matrices.
#    If the BVH file uses Z-up but we import with Y-up, all bone rotations are
#    off by 90 degrees. No formula can fix incorrect input data.
#
# 2. For Mixamo targets: the root bone IS the Hips. Setting root rotation to
#    zero destroys the Hips rotation data, breaking the entire skeleton.
#
# v6.0 APPROACH: Direct pose copy with proper axis handling.
#
# Key changes:
# - Auto-detect BVH axis convention from OFFSET values
# - Support both UE5 Quinn and Mixamo X Bot target skeletons
# - For Mixamo: direct 1:1 bone name matching (same skeleton = same rest poses)
# - For Mixamo root (Hips): transfer BOTH rotation AND translation
# - Use quaternion rotation mode to avoid Euler gimbal lock
# - Proper root bone handling: apply Hips rotation delta, not zero it out
...
def retarget_animation(bvh_armature, ref_armature, scale_factor=1.0):
    ...
    For each frame and each mapped bone:
      ...
      2. Apply delta to target bone's rest rotation
      3. Compute pose_delta using hierarchical formula
      4. Set pb.rotation_quaternion from pose_delta
    ...
    for fi in range(num_frames):
        ...
        if root_pb:
            if root_is_mapped and root_bvh_name:
                ...
                # Transfer both rotation delta AND root motion translation
                delta_root = bvh_delta.get(root_bvh_name, I3)
                root_rest = ref_rest_rot.get(root_bone_name, I3)

                # Apply rotation delta to root bone
                ...
                # Root is NOT mapped (e.g., UE5: "root" bone, no BVH equivalent)
                # Just apply root motion translation, zero rotation
                if bvh_hips and bvh_hips_ref_pos:
                    hips_pos = bvh_hips.matrix.translation
                    disp = hips_pos - bvh_hips_ref_pos
                    disp_corrected = forward_rotation @ disp
                    ...
                    root_pb.location = (0, 0, 0)
                root_pb.rotation_quaternion = (1, 0, 0, 0)  # Identity
        ...
        for pb in get_bones_in_hierarchy_order(ref_armature):
            ...
            if bvh_name and bvh_name in bvh_armature.pose.bones:
                ...
                if pb.parent:
                    # Find BVH bone mapped to target parent
                    parent_bvh_name = bone_map.get(pb.parent.name)
                    delta_parent = bvh_delta.get(parent_bvh_name, I3) if parent_bvh_name else I3

                    # If parent is the root bone and root is mapped,
                    # use root's delta as parent delta
                    if pb.parent.name == root_bone_name and root_is_mapped and root_bvh_name:
                        delta_parent = bvh_delta.get(root_bvh_name, I3)
                    ...
                    # Root-level bone (shouldn't happen if root is handled above)
                    pose_delta = ue5_rest.inverted() @ delta_bone @ ue5_rest

                # Set quaternion rotation directly (no Euler conversion issues)
                q = pose_delta.to_quaternion()
                # Ensure consistent quaternion sign (avoid flips)
                if q.w < 0:
                    q = -q
